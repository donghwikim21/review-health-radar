 ...
 */
export function computeConfidence(narrative: RawNarrative, facts: Fact[]): Confidence {
  ...
  const overall = statistical * (0.5 + 0.5 * reasoning);
  ...
  return {
    ...
    method:
      "overall = statistical × (0.5 + 0.5 × reasoning). Statistical confidence is derived in code from the sample size and reliability of the cited facts; reasoning confidence is the model's own. Overall is capped by statistical, so the narrative cannot sound more certain than the data supports.",
  };
}
